<template>
  ...
  <div class="live-page">
            ...
            <div class="sms-content" v-for="con in msg.content">
              ...
              <div class="content-text custom" v-if="con.type==msg.MSG_ELEMENT_TYPE.CUSTOM">
                <div class="custom" v-for="cus in con.custom">
                  ...
                  <div class="custom-video is-pc-video" v-if="cus.type == 'VIDEO'" @click="videoClick($event, 'id-'+cus.id)">
                    <video class="video-js vjs-default-skin"
                           :id="'id-'+cus.id" :poster="parseVideoPoster(cus.src)"  :data-source="cus.src" :data-id="'id-'+cus.id">
                  ...
                  <div class="custom-video mobile" v-if="1==2 && cus.type == 'VIDEO' && !isPC">
                    <video class="video-js vjs-default-skin" :id="'id-'+cus.id" :poster="`${cus.src}-preview.jpg!preview`"  :data-source="cus.src" :data-id="'id-'+cus.id">
                      <source :type="parseVideoType(cus.src)" :src="cus.src"/>
                    </video>
    ...
    <v-preview v-show="previewImg" :preview="previewImg" @prevPreviewImg="prevPreviewImg" @nextPreviewImg="nextPreviewImg"></v-preview>
    <!-- evaluate -->
  ...
  export default
  {
    ...
    methods: {
      ...
      scrollLoad(toward, limit, callback) {
        ...
        // 滚动加载
        this.$store.dispatch('fetchHistory', {cursor:cursor,limit:limit,toward:toward,lesson_sn:this.lesson.split('-')[0]}).then((msgList) => {
          //
          if (msgList.length == 0) {
            //
            if(toward == 'prev'){
