  ...
  <section class="user-money-out">
    <div class="money-con">
      <div class="money-forward">
        ...
        <div class="price" v-if="canShow">
          &#65509;{{remain}}
        </div>
        <div class="input">
          <input type="text" placeholder="请输入提现金额" v-model="price" @blur="priceBlur" />
        </div>
        <div>
          <button @click="startDrawCash" v-if="!doing">提现到微信零钱</button>
          <button v-if="doing">提现中...</button>
        </div>
      </div>
      <div class="letter">
        <ol>
